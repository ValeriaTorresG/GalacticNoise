...
class fit_data:
    ...
    def fit_sin(self, time, rms, ant_i, chan_i):
        ...
        def sinusoidal_model(x, amp, phase, mean):
            return amp * np.sin(12 * x + phase) + mean

        initial_guess = [guess_amp, guess_phase, guess_mean]
        bounds = ([0, -np.pi, -np.inf], [np.inf, np.pi, np.inf])

        popt, pcov = curve_fit(sinusoidal_model, bin_centers, bins, p0=initial_guess, bounds=bounds)
        est_amp, est_phase, est_mean = popt
        data_fit = sinusoidal_model(bin_centers, est_amp, est_phase, est_mean)
    ...
    def plot_pol(self, ax, ant_i, chan_i):
        ...
        ax.scatter(time, average_rms,alpha=0.5, c=colors[0])
    ...
    def plot_rms(self):
        for chan_i in range(self.pol):
            ...
            plt.savefig(f'plot_{chan_i+1}_otra.png', dpi=360)
            plt.close()
        self.logger.info('-')
